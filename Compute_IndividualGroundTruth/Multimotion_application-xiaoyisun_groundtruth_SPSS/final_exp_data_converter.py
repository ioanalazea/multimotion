...
def convert_experiment(survey_path, df_out_path):
    ...
    # Add a condition to remove specific elements
    elements_to_remove = ["Survey_L", "Survey_I", "Survey_Y"]
    slides = [slide for slide in slides if slide not in elements_to_remove]

    emotions = sorted(set(
        i[i.find('"') + 1: i.rfind('"')] for i in responses.loc[:, [next(iter(slides)) in i for i in responses.columns]]))
    ...
    df['stimulus'] = df['stimulus'].replace(mapping)
